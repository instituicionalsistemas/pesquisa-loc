...
const UserHomePage: React.FC = () => {
  ...
  const visibleCampaigns = useMemo(() => {
    return campaigns.filter(campaign => {
        // Se a campanha não tem ponto de ativação, sempre mostra
        if (!campaign.activationLatitude || !campaign.activationLongitude || !campaign.activationRadius) {
            return true;
        }
        // Se tem ponto de ativação, mas a localização do usuário ainda não foi obtida, não mostra
        if (!currentPosition) {
            return false;
        }
        // Calcula a distância e verifica se está dentro do raio
        ...
        return distance <= campaign.activationRadius;
    });
