...
const CampaignEditorPage: React.FC = () => {
  ...
  const [campaign, setCampaign] = useState<Partial<Campaign>>({
    ...
    endTime: '',
    finalRedirectUrl: '',
    activationLatitude: undefined,
    activationLongitude: undefined,
    activationRadius: undefined,
  });
    ...
    const fetchData = async () => {
        ...
        if (isEditing) {
          ...
          if (existingCampaign) {
            ...
            setIsActivationPointEnabled(!!existingCampaign.activationLatitude);
          } else {
  ...
  const handleSave = async () => {
    ...
    const campaignDataForDb = {
        ...
        latitude_ativacao: isActivationPointEnabled ? campaign.activationLatitude : null,
        longitude_ativacao: isActivationPointEnabled ? campaign.activationLongitude : null,
        raio_ativacao_metros: isActivationPointEnabled ? campaign.activationRadius : null,
    };
  ...
  // Effect for map setup and teardown
  useEffect(() => {
      if (isActivationPointEnabled && step === 1) {
          const timer = setTimeout(() => {
              if (document.getElementById('activation-map') && !mapRef.current) {
                  const L = (window as any).L;

                  const initialCoords: [number, number] = [
                      campaign.activationLatitude || -15.793889,
                      campaign.activationLongitude || -47.882778
                  ];
                  const initialZoom = campaign.activationLatitude ? 15 : 4;
                  
                  const map = L.map('activation-map').setView(initialCoords, initialZoom);
                  mapRef.current = map;
                  ...
                  map.on('click', (e: any) => {
                      ...
                      setCampaign(prev => ({
                          ...prev,
                          activationLatitude: lat,
                          activationLongitude: lng
                      }));
                  });
              }
          }, 100);
  ...
  }, [isActivationPointEnabled, step]);

  // Effect for updating map objects based on state
  useEffect(() => {
      if (mapRef.current && isActivationPointEnabled && step === 1) {
          const L = (window as any).L;
          const lat = campaign.activationLatitude;
          const lng = campaign.activationLongitude;
          const radius = campaign.activationRadius || 500;
          ...
          if (lat && lng) {
              ...
              if (circleRef.current) {
                  circleRef.current.setLatLng(latLng).setRadius(radius);
              } else {
              ...
              if (markerRef.current) {
                  ...
                  markerRef.current.on('dragend', (e: any) => {
                      ...
                      setCampaign(prev => ({
                          ...prev,
                          activationLatitude: newLatLng.lat,
                          activationLongitude: newLatLng.lng,
                      }));
                  });
              }
              ...
              // If no coords, remove marker and circle
              if(markerRef.current) {
      ...
      step,
      campaign.activationLatitude, 
      campaign.activationLongitude, 
      campaign.activationRadius
  ]);
            ...
            <div className="border border-light-border dark:border-dark-border rounded-lg">
                ...
                {isActivationPointEnabled && (
                  <div className="p-4 bg-gray-50 dark:bg-dark-background border-t border-light-border dark:border-dark-border">
                      <label className="block text-sm font-medium mb-1">Ponto de Ativação no Mapa</label>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Clique no mapa para definir o ponto central da campanha. Você pode arrastar o marcador para ajustar a posição.</p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                          <div>
                              <label htmlFor="displayLatitude" className="block text-sm font-medium mb-1">Latitude</label>
                              <input type="text" id="displayLatitude" value={campaign.activationLatitude?.toFixed(6) || ''} className="w-full input-style bg-gray-100 dark:bg-dark-border" readOnly placeholder="Selecione no mapa" />
                          </div>
                          <div>
                              <label htmlFor="displayLongitude" className="block text-sm font-medium mb-1">Longitude</label>
                              <input type="text" id="displayLongitude" value={campaign.activationLongitude?.toFixed(6) || ''} className="w-full input-style bg-gray-100 dark:bg-dark-border" readOnly placeholder="Selecione no mapa" />
                          </div>
                          <div>
                              <label htmlFor="activationRadius" className="block text-sm font-medium mb-1">Raio (metros)</label>
                              <input type="number" name="activationRadius" id="activationRadius" value={campaign.activationRadius || ''} onChange={handleDetailsChange} className="w-full input-style" placeholder="500" />
                          </div>
                      </div>
                  </div>
                )}
