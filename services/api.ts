...
import type { Admin, Campaign, Company, Researcher, SurveyResponse, Voucher, Question, QuestionOption, LocationPoint } from '../types';
...
const mapCampaignFromDb = (dbCampaign: any, allQuestionsData: any[], allOptionsData: any[], companies: any[], researchers: any[]): Campaign => {
    ...
    // The `respostas_pesquisas` can be an array of objects or just a number depending on how you select.
    ...
    return {
        ...
        activationLatitude: dbCampaign.latitude_ativacao,
        activationLongitude: dbCampaign.longitude_ativacao,
        activationRadius: dbCampaign.raio_ativacao_metros,
    } as any;
};

const mapCampaignForManagement = (dbCampaign: any, companies: any[], researchers: any[]): Campaign => {
    const responseCount = Array.isArray(dbCampaign.respostas_pesquisas) ? dbCampaign.respostas_pesquisas[0]?.count ?? 0 : 0;
...
export const getCampaignsForManagement = async (): Promise<Campaign[]> => {
    const [{ data: campaignsData, error }, companiesData, researchersData] = await Promise.all([
    ...
    return campaignsData.map(c => mapCampaignForManagement(c, companiesData, researchersData));
};
...
export const getPaginatedCampaigns = async (page: number, pageSize: number, filters: { status: string; companyId: string; searchQuery: string; researcherId?: string; }): Promise<{data: Campaign[], count: number}> => {
    ...
    if (error) throw error;
     ...
     const [questionsData, optionsData, companiesData, researchersData] = await Promise.all([
        fetchData('perguntas'),
        fetchData('opcoes_perguntas'),
        fetchData('campanhas_empresas'),
        fetchData('campanhas_pesquisadores'),
    ...
    const mappedData = campaignsData.map(c => mapCampaignFromDb(c, questionsData, optionsData, companiesData, researchersData));
    return { data: mappedData, count: count ?? 0 };
};
